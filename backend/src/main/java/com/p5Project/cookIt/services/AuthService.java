...
public class AuthService {
    ...
    public void forgotPassword(String email) {
        ...
        String body = String.format(
                "<html><body style=\"margin:0;padding:0;background:#fff7f3;font-family:Arial,sans-serif;color:#222;\">" +
                        "<div style=\"padding:32px 16px;\"><div style=\"max-width:720px;margin:0 auto;background:#fff;border-radius:22px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.12);\">" +
                        ...
                        "<div style=\"display:inline-block;padding:8px 14px;border-radius:999px;background:rgba(243,65,0,0.08);color:#f34100;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:18px\">Recuperacao de senha</div>" +
                        "<h1 style=\"margin:0 0 16px;font-size:28px;line-height:1.2;color:#1f1f1f\">Ola, %s!</h1>" +
                        "<p style=\"margin:0 0 18px;color:#555;font-size:18px;line-height:1.7\">Seu codigo para redefinir a senha e:</p>" +
                        "<div style=\"display:inline-block;font-size:30px;letter-spacing:6px;background:#f5f5f5;padding:18px 24px;border-radius:16px;margin-bottom:18px;font-weight:700;border:1px solid rgba(0,0,0,0.06)\">%s</div>" +
                        "<p style=\"margin:0;color:#777;font-size:16px;line-height:1.6\">Esse codigo expira em 15 minutos.</p>" +
    ...
    private void sendVerificationEmail(User user) {
        ...
        String body = String.format(
                "<html><body style=\"margin:0;padding:0;background:#fff7f3;font-family:Arial,sans-serif;color:#222;\">" +
                        ...
                        "<div style=\"height:14px;background:linear-gradient(90deg,#ff7a45 0%%,#f34100 50%%,#ffb36b 100%%)\"></div>" +
                        "<div style=\"padding:36px 34px 34px;text-align:center\">" +
                        "<div style=\"display:inline-block;padding:8px 14px;border-radius:999px;background:rgba(243,65,0,0.08);color:#f34100;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:18px\">Confirmacao de email</div>" +
                        "<h1 style=\"margin:0 0 16px;font-size:30px;line-height:1.2;color:#1f1f1f\">Ola, %s!</h1>" +
                        "<p style=\"margin:0 0 18px;color:#555;font-size:18px;line-height:1.7\">Obrigado por se cadastrar no <strong>Cook-It</strong>.</p>" +
                        "<p style=\"margin:0 0 18px\"><a href=\"%s\" style=\"display:inline-block;padding:16px 28px;background:linear-gradient(90deg,#ff7a45 0%%,#f34100 100%%);color:#fff;text-decoration:none;border-radius:14px;font-weight:700;font-size:18px;\">Confirmar email</a></p>" +
    ...
    private void ensureEmailVerificationCooldown(User user) {
        emailVerificationTokenRepository.findTopByUserOrderByCreatedAtDesc(user)
                .ifPresent(token -> ensureCooldownExpired(token.getCreatedAt(), 5,
                        "Voce ja solicitou um novo e-mail de confirmacao recentemente. Tente novamente em 5 minutos."));
    }
    ...
    private void ensurePasswordResetCooldown(User user) {
        passwordResetTokenRepository.findTopByUserOrderByCreatedAtDesc(user)
                .ifPresent(token -> ensureCooldownExpired(token.getCreatedAt(), 3,
                        "Voce ja solicitou uma redefinicao de senha recentemente. Tente novamente em 3 minutos."));
    }
    ...
    private PasswordResetToken requireValidResetToken(String token) {
        PasswordResetToken resetToken = passwordResetTokenRepository.findByToken(token)
                .orElseThrow(() -> new InvalidTokenException("Codigo invalido"));
